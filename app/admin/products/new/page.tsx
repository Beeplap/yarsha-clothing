...
export default function NewProductPage() {
  ...
  const [selectedColors, setSelectedColors] = useState<string[]>(["Core Black", "Cloud White"]);
  ...
  // Auto-generate SKU / Article Code & Slug
  const handleNameChange = (val: string) => {
    const slug = val.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)+/g, "");
    const generatedSku = val ? `YR-${val.slice(0, 3).toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}` : "";

    setFormData((prev) => ({
      ...prev,
      name: val,
      slug,
      article_code: prev.article_code || generatedSku,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let imageUrls: string[] = [...additionalImages];

      // 1. Upload Main File Image if exists
      if (imageFile) {
        ...
        imageUrls.unshift(imagePreview);
      }
      ...
      >
        <div>
          <span style={{ fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "var(--accent)" }}>
            Adidas Enterprise PIM Protocol
          </span>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="admin-form-card" style={{ padding: "2rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
              <div className="auth-field">
                ...
                <input
                  ...
                  placeholder="e.g. Ultraboost Light Running Shoes"
                  required
            ...
            <div className="auth-field" style={{ marginTop: "1.5rem" }}>
              ...
              <textarea
                ...
                rows={5}
                placeholder="Engineered for high performance and everyday comfort..."
                className="auth-input"
              />
            </div>
          </div>
            ...
            {imagePreview && (
              <div style={{ marginBottom: "2rem" }}>
                ...
                <img
                  src={imagePreview}
                  alt="Primary Preview"
                  style={{ width: "160px", height: "160px", objectFit: "cover", borderRadius: "0.5rem", border: "1px solid var(--color-neutral-300)" }}
                />
              </div>
            )}
            ...
            {additionalImages.length > 0 && (
              <div style={{ marginTop: "1.5rem" }}>
                <label style={{ fontWeight: 600, fontSize: "0.875rem", display: "block", marginBottom: "0.75rem" }}>
                  Attached Image Gallery ({additionalImages.length}):
                </label>
                  ...
                  {additionalImages.map((url, idx) => (
                    <div key={idx} style={{ position: "relative" }}>
                      <img
                        src={url}
                        alt={`Gallery ${idx + 1}`}
                        style={{ width: "100px", height: "100px", objectFit: "cover", borderRadius: "0.4rem" }}
                      />
                      <button
                        type="button"
            ...
            <div style={{ display: "flex", gap: "2rem", alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, padding: "1.5rem", background: "color-mix(in srgb, var(--foreground) 3%, transparent)", borderRadius: "8px" }}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: "0.75rem", fontSize: "0.9rem" }}>
                  ...
                  <li><strong>Slug:</strong> /products/{formData.slug || "pending"}</li>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "2rem", paddingTop: "1.5rem", borderTop: "1px solid var(--color-neutral-200)" }}>
          ...
          {activeTab !== "preview" ? (
            ...
            >
              {submitting ? "Publishing Product..." : "Confirm & Save Product"}
            </button>
          )}
        </div>
      </form>
