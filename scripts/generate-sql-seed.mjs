...
function generateSQL() {
  const lines = [
    "-- ==============================================",
    "-- MOCK SEED: 200 Wearable Products for Yarsha Wears",
    "-- ==============================================",
    "",
    "INSERT INTO public.products (name, slug, description, price, compare_at_price, stock_quantity, images, is_featured)",
    "VALUES",
  ];
  ...
  for (let i = 1; i <= 200; i++) {
    ...
    const name = `Yarsha ${adj} ${noun} Vol. ${i}`.replace(/'/g, "''");
    const slug = `yarsha-${adj.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${noun.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${now}-${i}`;
    const desc = `Official Yarsha Wears collection. The ${name} features high-density fabric, ergonomic fit, and durable craftsmanship designed for daily wear in Nepal.`.replace(/'/g, "''");
    ...
    const imagesArray = `ARRAY['${img1}', '${img2}']`;

    const isFeatured = i <= 16 || i % 8 === 0 ? "true" : "false";
    const stockQuantity = 20 + (i % 65);

    valueRows.push(
      `  ('${name}', '${slug}', '${desc}', ${basePrice}, ${compareAtPrice}, ${stockQuantity}, ${imagesArray}, ${isFeatured})`
    );
  }
  ...
  const targetPath = path.join(process.cwd(), "supabase", "migrations", "20260721000001_seed_200_products.sql");
  fs.writeFileSync(targetPath, sqlContent, "utf-8");

  console.log(`✅ Generated 200 Products SQL file at: ${targetPath}`);
}
